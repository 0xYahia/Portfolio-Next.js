...
const ProjectCard = ({
  ...
  return (
    ...
    >
      <Tilt
        scale={1}
        className="bg-violet-950 p-5 rounded-2xl  sm:w-[360px] w-full "
      >
        <div className="relative w-full h-[230px]  md:col-span-2 sm:col-span-4">
          <Image
            src={image}
            alt={name}
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {tags.map((tag) => (
            <p
              key={tag.name}
              className={`text-[14px] ${tag.color} flex justify-start py-1 px-2 rounded-md`}
            >
              # {tag.name}
            </p>
          ))}
